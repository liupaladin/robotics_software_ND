...
void visualization()
{
    ...
    vector<int> unknown_mat_x, unknown_mat_y;
    vector<int> occupied_mat_x, occupied_mat_y;
    vector<int> free_mat_x, free_mat_y;
    ...
    for (int x = 0; x < mapWidth / gridWidth; x++) {
        for (int y = 0; y < mapHeight / gridHeight; y++) {
            if (l[x][y] == 0) {
                unknown_mat_x.push_back(x);
                unknown_mat_y.push_back(y);
                // plt::plot({ x }, { y }, "g.");
            } else if (l[x][y] > 0) {
                occupied_mat_x.push_back(x);
                occupied_mat_y.push_back(y);
                // plt::plot({ x }, { y }, "k.");
            } else  {
                free_mat_x.push_back(x);
                free_mat_y.push_back(y); 
                // plt::plot({ x }, { y }, "r.");           
            }
        }
    }
    ...
    plt::plot(unknown_mat_x, unknown_mat_y, "g.");
    plt::plot(occupied_mat_x, occupied_mat_y,"k.");
    plt::plot(free_mat_x, free_mat_y, "r.");
